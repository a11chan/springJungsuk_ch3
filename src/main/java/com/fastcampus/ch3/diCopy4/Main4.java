...
//        //map에 저장된 객체의 iv 중에 @Resource 가 있으면
//        //map에서 iv의 이름에 맞는 객체를 찾아 연결(객체의 주소를 iv에 저장)
//        try {
...
//    }
    ...
    private void doAutowired() {
        //map에 저장된 객체의 iv 중에 @Autowired가 있으면
        //map에서 iv의 타입에 맞는 객체를 찾아 연결(객체의 주소를 iv에 저장)
        try {
            for(Object bean : map.values()) {
                for(Field fld : bean.getClass().getDeclaredFields()) {
    ...
    private void doComponentScan() {
        try {
            ...
            // 2. 반복문으로 클래스를 하나씩 읽어와서 @Component이 있는지 확인
            // 3. @Component가 있으면 객체를 생성해서 map에 저장
